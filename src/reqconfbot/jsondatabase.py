...
class ServerData:
    KEY_FORM_CHANNEL_ID: Final[str] = "form_channel_id"
    KEY_SERVER_ID: Final[str] = "server_id"
    ...
    def fromDict(cls, server_id: str, json_dict: dict[str, int | None]) -> ServerData:
        return ServerData(
            ...
            form_channel_id=json_dict[cls.KEY_FORM_CHANNEL_ID]
        )

    def __init__(self, server_id: int, *, form_channel_id: int = None, ) -> None:
    ...
    def write(self) -> tuple[str, dict[str, int]]:
        return (
            self.__writeKey(),
            {
                self.KEY_FORM_CHANNEL_ID: self.form_channel_id
            }
        )
